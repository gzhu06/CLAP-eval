...
if __name__ == '__main__':
    ...
    encoder_model = AudioEncoder(encoder_config)
    ...
    x = jnp.ones((batch_size, seq_len, patch_size))
    mask = jnp.ones((batch_size, seq_len))
    time_inds = repeat(jnp.arange(seq_len) // 8, ' s -> b s', b=batch_size)
    freq_inds = repeat(jnp.arange(seq_len) % 8, ' s -> b s', b=batch_size)

    encoder_params = encoder_model.init(rngs={'params': jax.random.PRNGKey(0), 'dropout': jax.random.PRNGKey(0)},
        x=x, mask=mask, time_inds=time_inds, freq_inds=freq_inds, deterministic=True)['params']
    
    encoder_out = encoder_model.apply({'params': encoder_params},
        x=x, mask=mask, time_inds=time_inds, freq_inds=freq_inds,
        deterministic=True)
    
    decoder_config = encoder_config
    decoder_model = AudioDecoder(decoder_config)

    restore_time_inds = time_inds
    restore_freq_inds = freq_inds

    restore_mask = mask

    decoder_params = decoder_model.init(rngs={'params': jax.random.PRNGKey(0), 'dropout': jax.random.PRNGKey(0)},
        x=x, mask=mask, time_inds=time_inds, freq_inds=freq_inds, restore_time_inds=restore_time_inds, restore_freq_inds=restore_freq_inds, restore_mask=restore_mask,
        deterministic=True)['params']
    
    decoder_out = decoder_model.apply({'params': decoder_params},
        x=x, mask=mask, time_inds=time_inds, freq_inds=freq_inds, restore_time_inds=restore_time_inds, restore_freq_inds=restore_freq_inds, restore_mask=restore_mask,
        deterministic=True)


    mae_config = AudioMAEConfig(
        encoder_config=encoder_config,
        decoder_config=decoder_config,
    )

    mae_model = AudioMAE(mae_config)

    mae_params = mae_model.init(rngs={'params': jax.random.PRNGKey(0), 'dropout': jax.random.PRNGKey(0)},
        x=x, mask=mask, time_inds=time_inds, freq_inds=freq_inds, restore_time_inds=restore_time_inds, restore_freq_inds=restore_freq_inds, restore_mask=restore_mask,
        deterministic=True)['params']

    mae_out = mae_model.apply({'params': mae_params},
        x=x, mask=mask, time_inds=time_inds, freq_inds=freq_inds, restore_time_inds=restore_time_inds, restore_freq_inds=restore_freq_inds, restore_mask=restore_mask,
        deterministic=True)
