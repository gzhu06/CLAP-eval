...
class AttentionPooler(nn.Module):
    ...
    def __call__(
        ...
        q = self.param('query', jax.nn.initializers.normal(0.02), (audio_hidden_state.shape[-1],)) # TODO(jd) TODO TODO init?
...
class CACO(nn.Module):
    ...
    def __call__(
        ...
        if self.caco_config.projection_size is None:
            assert audio_embedding.shape[-1] != text_embedding.shape[-1] #TODO(jd) should be == ????
        else:
