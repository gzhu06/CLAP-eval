...
class CACO(nn.Module):
    ...
    def setup(self):
        ...
        self.audio_proj = nn.Dense(
            proj_size,
            dtype=self.caco_config.dtype
        ) # TODO(jd) kernel inits?
        self.text_proj = nn.Dense(
            proj_size,
            dtype=self.caco_config.dtype
        )
        self.audio_pad_width = 2 if self.audio_module.config.use_dist_token else 1
    ...
    def __call__(
        ...
        post_audio_mask = jnp.pad(audio_mask, [[0, 0], [self.audio_pad_width, 0]], constant_values=1)

        audio_embedding, audio_hidden_state = self.audio_module(
